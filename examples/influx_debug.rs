...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Time range: {} to {}", start_time.format("%Y-%m-%d %H:%M:%S"), end_time.format("%Y-%m-%d %H:%M:%S"));
    println!("Query: {}", query);
    println!();
    ...
    match client.json_query(ReadQuery::new(query)).await {
        Ok(read_result) => {
            ...
            if record_count == 0 {
                ...
                println!("\nFound {} heart rate records in the last 7 days", record_count);
            }
        }
        Err(e) => {
                                    ...
                                    for value_row in values_array {
                                        if let Some(value_array) = value_row.as_array() {
                                            if let Some(count_value) = value_array.get(1) {
                                                println!("Total HeartRate records: {}", count_value);
                                            }
                                        }
                                    }
                                }
